...
struct BibleReference: Equatable {
    ...
    /// True when the passage is short enough to display inline (≤5 verses).
    var isShort: Bool { verseCount <= 5 }
}
...
func parseBibleReference(_ input: String) throws -> BibleReference {
    ...
    if let m = ref.firstMatch(of: /^(?P<book>.+?)\s+(?P<cs>\d+):(?P<vs>\d+)\s*[-–]\s*(?P<ce>\d+):(?P<ve>\d+)$/i) {
    ...
    if let m = ref.firstMatch(of: /^(?P<book>.+?)\s+(?P<ch>\d+):(?P<vs>\d+)\s*[-–]\s*(?P<ve>\d+)$/i) {
        let book = try resolveBook(String(m.book))
        let ch = Int(m.ch)!
        return BibleReference(book: book,
    ...
    if let m = ref.firstMatch(of: /^(?P<book>.+?)\s+(?P<ch>\d+):(?P<vs>\d+)$/i) {
        let book = try resolveBook(String(m.book))
        let ch = Int(m.ch)!
        let vs = Int(m.vs)!
    ...
    if let m = ref.firstMatch(of: /^(?P<book>.+?)\s+(?P<cs>\d+)\s*[-–]\s*(?P<ce>\d+)$/i) {
        let book = try resolveBook(String(m.book))
        return BibleReference(book: book,
                              chapterStart: Int(m.cs)!, chapterEnd: Int(m.ce)!,
    ...
    if let m = ref.firstMatch(of: /^(?P<book>.+?)\s+(?P<ch>\d+)$/i) {
        let book = try resolveBook(String(m.book))
        let num = Int(m.ch)!
